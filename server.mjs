...
loadEnv();
const port=Number(process.env.PORT||4173), apiUrl='https://openspeech.bytedance.com/api/v3/auc/bigmodel/recognize/flash';
...
createServer(async(req,res)=>{if(req.method==='OPTIONS')return json(res,204,{});if(req.method==='POST'&&new URL(req.url,'http://localhost').pathname==='/api/asr')return asr(req,res);if(req.method==='GET')return staticFile(req,res);json(res,405,{error:'Method not allowed'})}).listen(port,'127.0.0.1',()=>console.log(`跟着说运行于 http://127.0.0.1:${port}，ASR 配置来自 ${envPath}`));
