...
正式口播时，文字下方整块区域用于播放当前句。电脑端按 `←` / `→` 切换上一句和下一句，按空格键或 `R` 重播当前句；手机端左滑立即停止当前音频并播放下一句，右滑立即停止当前音频并播放上一句，轻点播放区域则重播当前句。不再启动浏览器 Web Speech API，不需要 API Key，也不会额外申请浏览器语音识别权限。
